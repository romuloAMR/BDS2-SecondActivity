...
// heapify function
void BinTree::heapify(std::vector<int>& arr, int n, int i) {
    int largest = i;
    int left = 2 * i + 1;
    int right = 2 * i + 2;
...
// vector to list binary tree conversion
BinTree* BinTree::vectorToMaxHeap(std::vector<int>& arr) {
    if (arr.empty()) {
        return nullptr;
    }
